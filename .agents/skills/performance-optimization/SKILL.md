...
> Reads `.Codex/config.json` for `${tooling.*}` (gate commands), `${project.stagingUrl}` (PSI target), `${gates.*}` (thresholds), `${paths.*}` (search scopes). Project-specific SEO/route specifics loaded from `${overlay}/seo-supplement.md` if present.
...
**Step 4 — Auth/authz audit.** Re-read `.Codex/rules/backend.md` + `.Codex/rules/database.md` (or project equivalents). Verify per-route auth scoping. For RLS systems: confirm policies on every table; test anon read against sensitive tables (must deny).
...
This skill reads `.Codex/config.json` for:
...
To use in another project: copy `.Codex/skills/performance-optimization/`, point `.Codex/config.json` at the new tooling/paths/URLs, and optionally write `${overlay}/seo-supplement.md` for project-specific SEO/route specifics.
