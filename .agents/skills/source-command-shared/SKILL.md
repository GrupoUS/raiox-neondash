...
Use this skill when the user asks to run the migrated source command `_shared`.
...
Every command reads `.Codex/config.json` at start to resolve project-specific values. Pattern:

```bash
# read config (commands invoke via Bash or Read)
test -f .Codex/config.json && cat .Codex/config.json
```
...
Rules live at `.Codex/rules/<file>.md` and are project-authoritative. When a command or skill says "read `.Codex/rules/<file>.md`", read it directly:

```bash
RULE=frontend.md
cat ".Codex/rules/$RULE"
```

Available rules: `frontend.md`, `DESIGN.md`, `stability.md`, `seo.md`. See `.Codex/rules/README.md` for scope of each.
...
> **Spawn contract:** every `Agent()` / `Task()` invocation MUST inject the 5 mandatory context fields from `.Codex/skills/senior-prompt-engineer/references/agent-handoff-contracts.md § 1`. Do not duplicate the field list in command bodies — link only.
...
> `explorer` = custom agent (`.Codex/agents/explorer-agent.md`), NOT the built-in `Explore`. Use `subagent_type: "explorer"`.
...
| Continuing prior session | Read `.Codex/docs/evolution/HANDOFF.md` first | — |
...
6. **Maximum 5 spawns per user request** (per AGENTS.md stopping conditions). At 5 → checkpoint with user.
...
All agents — single, parallel-batch, or coordinator-managed — return findings using the canonical schema in `.Codex/skills/senior-prompt-engineer/references/agent-handoff-contracts.md`. Parallel batches additionally conform to `.Codex/skills/senior-prompt-engineer/references/parallel-batch-contracts.md` (shared columns + severity scale + consolidation rules).
...
|---|---|---|
| Stability checklist A-L | `.Codex/rules/stability.md` | Any code change |
| Render mode declaration | `.Codex/rules/frontend.md` | Page/route changes |
| Content Collections SSOT | `.Codex/rules/frontend.md` | Product/team copy edits |
| External redirect tri-sync | `.Codex/rules/frontend.md` | externalSiteUrl change |
| WhatsApp SSOT | `.Codex/rules/frontend.md` | WhatsApp URL/CTA edits |
| Design tokens / no hex | `.Codex/rules/DESIGN.md` | Style changes |
| Anti-patterns + debug triage | `.Codex/rules/stability.md` | Per-project bugs |
