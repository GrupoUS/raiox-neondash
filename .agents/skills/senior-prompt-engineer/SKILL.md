...
description: Canonical contract for Codex subagent design, handoff schemas, orchestrator spawn templates, parallel-batch return contracts, and coordinator failure-recovery rules. Use when authoring or editing .Codex/agents/*.md, designing multi-agent commands in .Codex/commands/, building handoff schemas, or when a parallel batch needs a shared return contract. Also covers prompt engineering for application-level LLM features (RAG, structured outputs, eval harnesses).
---

# Senior Prompt Engineer — Codex Agent Orchestration SSOT

> Single source of truth for **how Codex agents talk to each other**.
> Anchored to Anthropic official docs (sub-agents, skills) — see `references/agentic_system_design.md`.
> Project-agnostic: examples use placeholders. Host project supplies stack-specific rules via `.Codex/AGENTS.md` + `.Codex/rules/`.
...
1. **Subagent design** — what every `.Codex/agents/*.md` file must look like (frontmatter + body sections).
...
Every `.Codex/agents/<name>.md` must follow this shape:

```markdown
---
name: <unique-lowercase-hyphens>
description: <when Codex should delegate — front-load use case>
...
- Hardcoded project-specific values that belong in `.Codex/AGENTS.md` / `.Codex/rules/` / `.Codex/config.json` — agents inherit the same project rules as commands.

---

## 3. Description guidelines (auto-trigger reliability)

Per Anthropic skills doc, descriptions are loaded into context so Codex knows what's available. They power both:

1. **Auto-invocation** — Codex reads descriptions to decide when to delegate.
...
3. Max 5 spawns per user request (per `AGENTS.md § Stopping conditions`).
...
For migrating Codex API code or building new Anthropic SDK apps, prefer the `Codex-api` skill (it auto-triggers on `@anthropic-ai/sdk` imports).
...
| `confidence ≤ 2` proceeding without `[ASSUMED]` flag | Plans built on speculation | Per `AGENTS.md § Stopping conditions` — flag and ask user |
| Parallel batch with ad-hoc per-agent table shape | Manual consolidation, missed findings | Single shared findings schema (§ 6) |

---

Owner: project-scoped (`.Codex/skills/`).
