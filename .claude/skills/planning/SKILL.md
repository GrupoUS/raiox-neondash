...
> **Project-specific layer chain + verify commands** are loaded from `${overlay}/layer-map.md` (path resolved from `.claude/config.json::overlay`). If overlay is missing, the skill uses the generic template in § Step 2.
...
Below any threshold → evaluator returns specific failures → revise. Max 3 revision iterations before escalating to user (or coordinator BLOCKED → `/debug recover` per `.claude/CLAUDE.md § Stopping conditions`).
...
- [ ] Project cardinal rules honored — re-read `.claude/CLAUDE.md` cardinal rules before presenting
...
- `.claude/skills/senior-prompt-engineer/references/agent-handoff-contracts.md` — Context Handoff schema (planner → executor handoff)
- `.claude/skills/senior-prompt-engineer/references/parallel-batch-contracts.md` — Findings table + consolidation rules for parallel batches
- `.claude/CLAUDE.md` — host project's cardinal rules + routing matrix
- `.claude/config.json` — host project's paths / tooling / gates
